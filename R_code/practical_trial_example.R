...
## sample size calculation for APSAC study
lambda_as <- calculate_lambda(0.04, 0.12)
N_fix <- 2*((qnorm(0.975)-qnorm(0.2))/lambda_as)^2
...
beta <- 0.2
alpha_glob <- 0.025
design <- design(n_1 = 90, alpha_glob = alpha_glob, n_2 = 90, alpha_0 = 0.5, f = 3)
w_1 <- sqrt(design$n_1)
w_2 <- sqrt(design$n_2)
...
for(i_rule in 1:length(list_rec_rules)){
  ...
  for(i_n1 in 1:length(n1_values)){
    ...
    ## save results
    fwrite(as.data.table(results_rec_rule), paste0('results/apsac_results/', rec_rule, '_n1_', n_1, '.csv'))
    results_rec_rule <- NULL
  }
